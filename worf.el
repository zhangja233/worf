...
(defun worf-backward ()
  ...
  (interactive)
  (when (re-search-backward worf-regex-full nil t)
...
(defvar worf-refile-other-hook #'ignore
  "Allow to override `worf-refile-other'.")
...
directory, the attachments will be moved."
  (interactive "p")
  (or (funcall worf-refile-other-hook)
...
This is accomplished by putting it at the start of `org-refile-history'."
  (let ((bname (file-name-nondirectory fname)))
      ...
      (let ((default-target
             (cond
               ...
               ((save-excursion
                ...
                (format "Tasks (%s)" bname)))))
        (when (and default-target
                   (not (string= (car org-refile-history) default-target)))
...
(defvar worf-refile-archive-hook #'ignore
  "Allow to override `worf-refile-archive'.")
...
(defun worf--at-property-p ()
  ...
  (let ((case-fold-search t))
    (looking-at "^:\\(?:property\\|properties\\|end\\|logbook\\)")))
